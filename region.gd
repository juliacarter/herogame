...
#the faction that controls this region
var owner

#when this is <5, other faction can initiate takeover. failed Encounters reduce this.
#as this goes down, reduce effects of the region for owner
var control = 10


#Assets present in the region, by faction
var assets = {
...
#amount of influence each non-player faction needs to roll their next scheme
var influence_to_level = {
	"coalition": 100.0,
}
...
#check if faction has enough influence in the region. create the asset if it does
func try_asset(assetname, faction):
	pass

func make_asset(assetname, faction):
	if rules.data.assets.has(assetname):
		var assetdata = rules.data.assets[assetname]
		var type
		if rules.script_map.has(assetdata.type):
			type = rules.script_map[assetdata.type]
		else:
			type = Asset
		var asset = type.new(rules, assetdata)
		add_asset(asset)
...
func add_asset(asset):
	...
	assets[asset.faction].append(asset)
	
func remove_asset(asset):
	if assets.has(asset.faction):
		var i = assets[asset.faction].find(asset)
		if i != -1:
			assets[asset.faction].pop_at(i)
...
func asset_think(delta):
	for faction in assets:
		var options = assets[faction]
