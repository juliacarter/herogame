...
func load_visuals():
	for key in visuals_to_load:
		var animdata = visuals_to_load[key]
		var anim = VisualEffect.new(animdata)
		visual_effects.merge({
			key: anim
		})
...
func make_prereq(prereqdata):
	if prereq_objects.has(prereqdata.type):
